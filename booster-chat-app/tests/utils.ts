...
function appStackName(): string {
  return `booster-chat-app-test-app`
}
...
export const postUser = async ({ email, password, role }: { email: string; password: string; role: string }) => {
...
export const createMessages = (): Array<Message> => {
  return [1, 2, 3, 4, 5].map(i => {
    return new Message(
        ...
        MessageType.CHAT,
        random.word(),
        new UserProfile(new UUID(i), random.word(), random.image())
    )
  })
}
