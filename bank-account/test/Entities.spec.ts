...
import { WithdrawPerformed } from '../src/events/WithdrawPerformed'
...
describe('BankAccount', () => {
  ...
  describe('reduceWithdrawPerformed', () => {
    it('should decrease the account balance in the given withdraw amount', () => {
      const givenAmount = 10
      const someWithdraw = new WithdrawPerformed(givenIban, givenAmount)
      const accountWithBalanceZero = new BankAccount(givenIban, givenOwner, 20)

      const result = BankAccount.reduceWithdrawPerformed(someWithdraw, accountWithBalanceZero)
